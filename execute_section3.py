"""
execute_section3.py
Runs Section 3 code cells (NLP preprocessing) with the shared namespace
that already has df loaded from Sections 1+2.
"""
...
# ── Rebuild namespace from scratch (same as Sections 1+2 would have built) ──
...
print(f"Executing Section 3: cells {sec3_start} to {sec4_start-1}")

# Find last exec count from previous sections
prev_exec = max(
...
print(f"   Cleaned_Text added to df: {'Cleaned_Text' in namespace['df'].columns}")
print(f"   Sample cleaned text: {namespace['df']['Cleaned_Text'].iloc[0]}")
